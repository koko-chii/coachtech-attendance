...
class AttendanceSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // 勤怠データを生成する対象（一般スタッフのユーザー1とユーザー2）を配列にまとめる
        $staffMembers = [$user1, $user2];

        // 各スタッフごとにダミーデータを生成するループ処理
        foreach ($staffMembers as $staff) {
            ...
            for ($i = 5; $i >= 1; $i--) {
                ...
                while ($createdDays < 15) {
                    if (!$monthDate->isWeekend()) {
                        $this->createRecord($staff->id, $monthDate, '09:00:00', '18:00:00');
                        $createdDays++;
                    }
                    $monthDate->addDay();
                }
            ...
            foreach ($patterns as $pattern) {
                ...
                $this->createRecord($staff->id, $currentMonthDate, $pattern[0], $pattern[1]);
                $currentMonthDate->addDay();
            }
        }
    }
